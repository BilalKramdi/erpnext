...
class TranslatePOTests(unittest.TestCase):
	...
	def test_checkpoint_restores_added_translation_provenance(self):
		translated = SAMPLE.replace('msgstr ""\n\nmsgid "Company"', 'msgstr "Enregistrer {name}"\n\nmsgid "Company"', 1)
		_, entries = parse_po(translated)
		decisions, candidates = baseline_decisions(entries)
		self.assertEqual(candidates, [])
		entry = next(item for item in entries if item.msgid == "Save {name}")
		checkpoint = {
			entry.key: {
				"old_translations": {"msgstr": ""},
				"translations": [{"field": "msgstr", "value": "Enregistrer {name}"}],
				"explanation": "Bouton d’enregistrement.",
				"glossary_terms": [],
				"confidence": "high",
			}
		}
		restore_checkpoint_provenance(decisions, checkpoint)
		decision = next(item for item in decisions if item.entry is entry)
		self.assertEqual(decision.status, "traduction ajoutée")
		self.assertEqual(decision.old, {"msgstr": ""})
		self.assertEqual(decision.new, {"msgstr": "Enregistrer {name}"})
