...
def restore_checkpoint_provenance(decisions: list[Decision], checkpoint: dict[str, dict[str, Any]]) -> None:
	"""Mark translations recorded as initially empty in the checkpoint as added.

	This keeps the report meaningful after a successful run: on subsequent runs
	the PO fields are already populated and would otherwise all look pre-existing.
	"""
	for decision in decisions:
		record = checkpoint.get(decision.entry.key)
		if not isinstance(record, dict):
			continue
		original = record.get("old_translations")
		if not isinstance(original, dict):
			continue
		missing_names = [name for name, value in original.items() if value == ""]
		current = old_values(decision.entry)
		if not missing_names or any(not current.get(name) for name in missing_names):
			continue

		proposed_raw = record.get("translations", [])
		...
		added = {name: current[name] for name in missing_names}
		matches_proposal = all(proposed.get(name) == value for name, value in added.items())
		if matches_proposal:
			comment = record.get("explanation", "Traduction ajoutée lors d’une exécution précédente.")
		else:
			comment = "Traduction ajoutée puis corrigée ou validée manuellement après la proposition initiale."

		decision.old = {name: value for name, value in original.items() if isinstance(value, str)}
		decision.new = added
		decision.status = "traduction ajoutée"
		decision.comment = comment
		decision.glossary_terms = record.get("glossary_terms", [])
		decision.confidence = {"high": "élevé", "medium": "moyen", "low": "faible"}.get(
			record.get("confidence"), "moyen"
		)
		decision.errors = []
...
def build_parser() -> argparse.ArgumentParser:
	...
	parser.add_argument("po_file", nargs="?", type=Path, default=Path("erpnext/locale/fr.po"))
...
def main(argv: list[str] | None = None) -> int:
	...
	restore_checkpoint_provenance(decisions, checkpoint)
	try:
